    ...
    return (
        <div className="max-w-3xl mx-auto p-6">
            <div className="flex items-center justify-between mb-4">
                ...
                <form action={logout}>
                    <button type="submit" className="border rounded px-3 py-2">Logout</button>
                </form>
            </div>
            {supabaseError ? (
                            ...
                            {data?.map((row, idx) => {
                                ...
                                const summarizeUA = (uaRaw: string): string => {
                                    ...
                                    const browser = (() => {
                                        ...
                                        const safari = ua.includes("Safari") && !ua.includes("Chrome") ? `Safari ${versionSafari?.[1] ?? ""}`.trim() : "";
                                    ...
                                    const os = (() => {
                                        ...
                                        if (/iPhone|iPad|iPod/.test(ua)) {
                                            ...
                                            return `iOS${ios ? " " + (ios?.[1] ?? "").replace(/_/g, ".") : ""}`.trim();
                                        }
                                ...
                                const getDevice = (uaRaw: string): string => {
                                    const ua = uaRaw || "";
                                    return /Mobile|iPhone|Android|iPad|iPod/.test(ua) ? "Mobile" : "Desktop";
                                };
                                ...
                                return (
                                    <tr key={idx} className="border-b/50">
                                        {(["time", "ua", "device", "geo", "client_id"] as const).map((key) => {
                                            ...
                                            let display: string;
                                            if (key === "device") {
                                                const uaValue = (row as Record<string, unknown>)["ua"] as string | undefined;
                                                display = getDevice(String(uaValue ?? ""));
                                            } else if (key === "geo" && value && typeof value === "object") {
                                                const geo = value as Record<string, unknown>;
                                                const countryCode = (geo["country"] || geo["countryCode"]) as string | undefined;
                                                const city = (geo["city"] as string | undefined) || "";
                                                const flag = toFlagEmoji(countryCode);
                                                display = flag && city ? `${flag} ${city}` : (flag || city || "");
                                                if (!display) display = JSON.stringify(value);
                                            } else {
                                                display = typeof value === "object" ? JSON.stringify(value) : String(value ?? "");
                                            }
                                            const cellClass = key === "geo" ? "py-2 pr-4 align-middle" : "py-2 pr-4 align-top";
                                            return (
                                                <td key={key} className={cellClass}>
                                                    {key === "time" ? (
                                                        <TimeCell iso={String(value ?? "")} />
                                                    ) : key === "ua" ? (
                                                        <span title={String(value ?? "")} aria-label={String(value ?? "")}>
                                                            {truncate(summarizeUA(String(value ?? "")))}
                                                        </span>
                                                    ) : (
                                                        <span title={display} aria-label={display}>{truncate(display)}</span>
                                                    )}
                                                </td>
                                            );
                                        })}
